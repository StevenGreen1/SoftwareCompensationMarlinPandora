...
/**
 *  @file   MarlinPandoraSC/src/EnergyCorrectionSCLan.cc
 * 
...
EnergyCorrectionSCLan::EnergyCorrectionSCLan() //:
...
StatusCode EnergyCorrectionSCLan::MakeEnergyCorrections(const pandora::Cluster *const pCluster, float &correctedHadronicEnergy) const
{
  //std::cout << "Starts EnergyCorrectionSCLan" << std::endl;
...
//    const unsigned int nTrackAssociations = trackList.size();
    ...
    //if (C_global>1.2 || (maxDiff/(maxHitEnergy-minHitEnergy))<0.7 )
    //return STATUS_CODE_SUCCESS;
    ...
    */
    ...
    if (isECalCluster)
    {
        //EnergyCorrectionSCLan::ECalClusterEnergyCorrectionFunction(clusterCaloHitList, correctedHadronicEnergy);
        return STATUS_CODE_SUCCESS;
    }

    else if (isHCalCluster){
      EnergyCorrectionSCLan::SCClusterEnergyCorrectionFunction(clusterHADenergy,clusterCaloHitList, correctedHadronicEnergy);
      //hClusterE->Fill(clusterHADenergy);
    }
    else
{
      EnergyCorrectionSCLan::SCHCalECalSplitClusterEnergyCorrectionFunction(clusterHADenergy,clusterCaloHitList, correctedHadronicEnergy);
}    
    return STATUS_CODE_SUCCESS;
...
float FindDensity(float hitEnergy, float cellsize){
  cellsize /= 100;
  float volume = cellsize*cellsize*0.265;
  ...
  float rho = 0;
  const float hitEnergyDensity(hitEnergy/volume);

  for (int ibin = 0; ibin < NBIN; ibin++)
  {
      if (hitEnergyDensity>=lowDensity[ibin] && hitEnergyDensity<highDensity[ibin])
      {
          rho = (lowDensity[ibin]+highDensity[ibin])/2;
          if (ibin==(NBIN-1))
          {
              rho = 30;
          }
  ...
  //std::cout << "SoftComp2 HitE : " << hitEnergy << std::endl;
  //std::cout << "SoftComp2 : " << rho << std::endl;
...
StatusCode EnergyCorrectionSCLan::clusterType(const pandora::CaloHitList &caloHitList, bool &isECalCluster, bool &isHCalCluster) const
{
...
StatusCode EnergyCorrectionSCLan::ECalClusterEnergyCorrectionFunction(const pandora::CaloHitList &caloHitList, float &energyCorrection) const
{
    //std::cout << "========ECAL CONTAINED CLUSTER -> Calling EnergyCorrectionSCLan::ECalClusterEnergyCorrectionFunction========" << std::endl;
...
StatusCode EnergyCorrectionSCLan::SCClusterEnergyCorrectionFunction(float clusterEnergyEstimation, const pandora::CaloHitList &caloHitList, float &energyCorrection) const
{
  ...
  for(pandora::CaloHitList::const_iterator iter = caloHitList.begin() , endIter = caloHitList.end() ; endIter != iter ; ++iter)
	...
	//float cellthickness = pCaloHit->GetCellThickness();
...
StatusCode EnergyCorrectionSCLan::SCHCalECalSplitClusterEnergyCorrectionFunction(float clusterEnergyEstimation, const pandora::CaloHitList &caloHitList, float &energyCorrection) const
{

//    std::cout << "========SPLIT CLUSTER========" << std::endl;

//    std::cout << "m_SCEnergyConstants.at(0) : " << m_SCEnergyConstants1.at(0) << std::endl;
//    std::cout << "m_SCEnergyConstants.at(1) : " << m_SCEnergyConstants1.at(1) << std::endl;
//    std::cout << "m_SCEnergyConstants.at(2) : " << m_SCEnergyConstants1.at(2) << std::endl;
//    std::cout << "m_SCEnergyConstants.at(3) : " << m_SCEnergyConstants1.at(3) << std::endl;
//    std::cout << "m_SCEnergyConstants.at(4) : " << m_SCEnergyConstants1.at(4) << std::endl;
//    std::cout << "m_SCEnergyConstants.at(5) : " << m_SCEnergyConstants1.at(5) << std::endl;
//    std::cout << "m_SCEnergyConstants.at(6) : " << m_SCEnergyConstants1.at(6) << std::endl;
//    std::cout << "m_SCEnergyConstants.at(7) : " << m_SCEnergyConstants1.at(7) << std::endl;
//    std::cout << "m_SCEnergyConstants.at(8) : " << m_SCEnergyConstants1.at(8) << std::endl;
...
StatusCode EnergyCorrectionSCLan::ReadSettings(const TiXmlHandle xmlHandle)
{
